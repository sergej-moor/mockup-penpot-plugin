...
async function handlePluginMessage(message: PluginMessage): Promise<void> {
  ...
  switch (message.type) {
    ...
    case 'capture-screenshot':
      // Send acknowledgment back to UI to trigger screenshot capture
      sendMessage({ type: 'capture-screenshot' });
      break;
    ...
    case 'upload-screenshot':
      try {
        ...
        // Create a new rectangle shape with the screenshot as fill
        const rect = penpot.createRectangle();
        rect.x = penpot.viewport.center.x;
        rect.y = penpot.viewport.center.y;
        rect.resize(uploadedImage.width, uploadedImage.height);

        // Set the image as fill
        const imageFill: Fill & { type: 'image' } = {
          type: 'image',
          fillImage: uploadedImage,
          opacity: 1,
          blend: 'normal',
        };
        rect.fills = [imageFill];
